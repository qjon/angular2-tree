...
describe('NodeService', () => {
  ...
  beforeEach(() => {
    http = <Http>jasmine.createSpyObj('Http', ['delete', 'get', 'post', 'put']);
  ...
  describe('load', () => {
    it('should call http.get with proper params and return proper response', () => {
      ...
      http.get.and.returnValue(Observable.of(new Response(new ResponseOptions({body: response}))));

      service.load('5')
        .subscribe(handler);

      expect(http.get).toHaveBeenCalledWith('/api/nodes', {search: params});
  ...
  describe('add', () => {
    it('should call POST with proper params and return proper response', () => {
      ...
      http.post.and.returnValue(Observable.of(new Response(new ResponseOptions({body: newNode}))));
  ...
  describe('move', () => {
    it('should call POST with proper params', () => {
      http.put.and.returnValue(Observable.of(new Response(new ResponseOptions({}))));
  ...
  describe('update', () => {
    it('should call PUT with proper params', () => {
      http.put.and.returnValue(Observable.of(new Response(new ResponseOptions({body: node}))));
  ...
  describe('remove', () => {
    it('should call DELETE with proper params', () => {
      http.delete.and.returnValue(Observable.of(new Response(new ResponseOptions({}))));

      service.remove(node.id)
        .subscribe(handler);

      expect(http.delete).toHaveBeenCalledWith('/api/nodes', {body: {nodeId: node.id}});
    });
  });
});
